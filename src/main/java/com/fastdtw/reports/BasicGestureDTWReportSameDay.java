...
public class BasicGestureDTWReportSameDay {
	...
	public static void main(String[] args) {
	    ...
	    double reportlByUserSameDay[][] = new double[8][8];
		...
		for(int u=1; u<=8; u++) {
			...
			for(int d=1; d<=7; d++) {
				...
				reportlByUserSameDay = sumMatrixs(reportlByUserSameDay, testByUser(arrSamples));				
				reportlByUser = sumMatrixs(reportlByUser, addToUserReport(reportlByUserSameDay));
				reportlByUserSameDay = resetValues(reportlByUserSameDay);
			}			
			reportlFinal = sumMatrixs(reportlFinal, addToFinalReport(reportlByUser));			
			for(int t=0; t<8; t++) {
				for(int v=0; v<8; v++) {
					System.out.print("  " + roundToDecimals(reportlByUser[t][v]/7,2));
				}
				System.out.println("\n");
			}
			System.out.println("  ==============================================  ");
	...
	public static double[][] addToFinalReport(double[][] matrix) {
		...
		for(int i=0; i<8; i++) {
			for(int j=0; j<8; j++) {
				result[i][j] = (roundToDecimals((matrix[i][j]/7),2));
			}
		}
		return result;
	}
	////////////////////////////
	public static double[][] addToUserReport(double[][] matrix) {
		double result[][] = new double[8][8];
		for(int i=0; i<8; i++) {
			for(int j=0; j<8; j++) {
